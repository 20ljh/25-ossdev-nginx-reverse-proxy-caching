...
```
# 코드 다운로드
git clone [https://github.com/20ljh/25-ossdev-nginx-reverse-proxy-caching.git](https://github.com/20ljh/25-ossdev-nginx-reverse-proxy-caching.git)

# 프로젝트 폴더로 이동
cd 25-ossdev-nginx-reverse-proxy-caching
```
...
- 웹 브라우저(Chrome 권장)를 엽니다.
- **`F12`** 키를 눌러 **개발자 도구**를 열고 **[Network]** 탭을 클릭합니다.
- 주소창에 **`http://localhost`** 를 입력하고 접속합니다.
- **결과** : 화면이 즉시 뜨지 않고, 약 **5초 간의 지연(Loading)** 후 페이지가 나타납니다.

  - *이유 : NGINX에 저장된 캐시가 없어, 느린 백엔드 서버에서 데이터를 직접 가져왔기 때문입니다.*

### 2단계 : 캐싱 효과 확인 (Cache HIT)

- 브라우저의 **새로고침(F5)** 버튼을 누릅니다.
- **결과** : 지연 없이 **즉시(0.01초 이내)** 페이지가 로딩됩니다.
